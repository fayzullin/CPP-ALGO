    ...
    void solveSudoku(vector<vector<char>>& board) {
        for (int i = 0; i < 9; ++i)
        {
            for (int j = 0; j < 9; ++j)
            {
                ...
                {
                    auto [solvedBoard, isSolved] = solveSudokuImpl(board, {i, j});
                    if (!isSolved)
                    {
...
private:
    std::pair<vector<vector<char>>, bool> solveSudokuImpl(vector<vector<char>> board, coords currEmptyCell)
    {
        ...
        for (char i = '1'; i <= '9'; ++i)
        {
            ...
            auto [resultBoard, isSolved] = solveSudokuImpl(board, nextEmptyCell);
            if (isSolved)
            {
    ...
    bool isNumInCurrRow(char num, coords currCoords, const vector<vector<char>>& board)
    {
        for (int i = 0; i < 9; ++i)
        {
    ...
    bool isNumInCurrColumn(char num, coords currCoords, const vector<vector<char>>& board)
    {
        for (int i = 0; i < 9; ++i)
        {
    ...
    bool isNumInCurrSubBox(char num, coords currCoords, const vector<vector<char>>& board)
    {
    ...
    coords getNextEmptyCell(const vector<vector<char>>& board, coords currEmptyCell)
    {
    ...
    bool isCellEmpty(const vector<vector<char>>& board, coords cell)
    {
